import pytest
import allure
from small_FrontalGate.common.operation.user import register_user, login_user, get_one_user_info, update_user
...
class TestRegLogUpdate():
    ...
    def test_user_register_login_update_success(self, testcase_data):
        ...
        id = result.response.json().get("test_data")[0].get("id")
        step_3(id)
        assert result.success is True, result.error
    ...
    @pytest.mark.flaky(reruns=3, reruns_delay=1) # 标记失败重跑
